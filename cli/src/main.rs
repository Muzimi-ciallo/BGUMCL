...
async fn try_connect(port: u16) -> Result<LauncherClient, String> {
  ...
  if server_info.server_info.name != EXPECTED_SERVER_NAME {
    ...
    let _ = client.cancel().await;
    return Err(format!(
      "endpoint {endpoint} is not the BGUMCL MCP server (got `{actual_name}`)"
    ));
  }
...
fn run_sjmcl_deeplink() -> Result<(), String> {
  ...
  let mut command = {
    ...
    command.arg(RUN_BGUMCL_DEEPLINK);
    command
  };
  ...
  let mut command = {
    ...
    command.args(["/C", "start", "", RUN_BGUMCL_DEEPLINK]);
    command
  };

  command
    .status()
    .map_err(|err| format!("failed to open deeplink `{RUN_BGUMCL_DEEPLINK}`: {err}"))
    .and_then(|status| {
      if status.success() {
        Ok(())
      } else {
        Err(format!(
          "deeplink launcher exited with status {} for `{RUN_BGUMCL_DEEPLINK}`",
          status
        ))
      }
    })
