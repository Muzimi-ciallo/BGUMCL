...
REPO="${BGUMCL_REPO:-UNIkeEN/BGUMCL}"
...
Environment:
  BGUMCL_VERSION       Release version or tag to install. Defaults to the latest stable release.
  BGUMCL_REPO          GitHub repository to download from. Defaults to UNIkeEN/BGUMCL.
EOF
}
...
set_asset_name() {
  if [ "$INSTALL_MODE" = "portable" ]; then
    ASSET_NAME="BGUMCL_${VERSION}_linux_${ARCH}_portable"
  else
    ASSET_NAME="BGUMCL_${VERSION}_linux_${ARCH}.${PACKAGE_TYPE}"
  fi
}
...
for SOURCE in $SOURCE_ORDER; do
  ...
  log "Installing BGUMCL $VERSION for $ARCH"
...
[ "$DOWNLOADED" = "1" ] || fail "failed to download BGUMCL from available sources"
...
if [ "$INSTALL_MODE" = "portable" ]; then
  ...
  log "Installed portable BGUMCL to $PORTABLE_PATH"
  ...
  case ":${PATH:-}:" in
    ...
    *)
      ...
      log "Add it to PATH or run BGUMCL directly with: $PORTABLE_PATH"
      ;;
  esac
  ...
  log "BGUMCL $VERSION installed successfully."
  exit 0
fi
