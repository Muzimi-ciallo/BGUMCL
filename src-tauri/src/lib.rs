...
fn open_modpack_deeplink(handle: &tauri::AppHandle, path: &str) {
  let deep_link = format!("bgumcl://import-modpack?path={}", urlencoding::encode(path));
  ...
  let exit_code = {
    let builder = tauri::Builder::default()
      ...
      .plugin(tauri_plugin_single_instance::init(|app, args, _cwd| {
        ...
        for arg in &args {
          if arg.ends_with(".mrpack") && !arg.starts_with("bgumcl://") {
    ...
    instance::helpers::modpack::github::set_modpack_version,
      ...
      .setup(|app| {
        ...
        tokio::spawn(async move {
          utils::sys_info::send_statistics(format!("BGUMCL-{}", version), os, exe_sha256).await;
        });
      ...
      .unwrap_or_else(|e| {
        ...
        native_dialog::DialogBuilder::message()
          ...
          .set_text(format!("Cannot initialize BGUMCL due to an error:\n{e}"))
