...
pub async fn fetch_auth_url(app: &AppHandle, root: Url) -> BGUMCLResult<String> {
  let client = app.state::<reqwest::Client>();
  let response = client
    .get(root.clone())
    .send()
...
pub async fn refresh_and_update_auth_servers(app: &AppHandle) -> BGUMCLResult<()> {
