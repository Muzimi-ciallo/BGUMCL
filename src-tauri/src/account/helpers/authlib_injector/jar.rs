...
pub fn get_jar_path(app: &AppHandle) -> BGUMCLResult<PathBuf> {
  Ok(
    app
      .path()
...
) -> BGUMCLResult<AuthlibInjectorMeta> {
...
fn get_local_version(app: &AppHandle) -> BGUMCLResult<String> {
...
async fn download(app: &AppHandle, url: Url) -> BGUMCLResult<()> {
  let client = app.state::<reqwest::Client>();
  let jar_path = get_jar_path(app)?;
...
pub async fn check_authlib_jar(app: &AppHandle) -> BGUMCLResult<()> {
  let latest_meta = {
