...
pub async fn fetch_image(app: &AppHandle, url: String) -> BGUMCLResult<ImageWrapper> {
...
pub fn get_selected_player_info(app: &AppHandle) -> BGUMCLResult<PlayerInfo> {
...
pub fn add_player(app: &AppHandle, new_player: PlayerInfo) -> BGUMCLResult<()> {
...
pub fn get_player_by_id(app: &AppHandle, player_id: &str) -> BGUMCLResult<Option<PlayerInfo>> {
  let account_binding = app.state::<Mutex<AccountInfo>>();
  let account_state = account_binding.lock()?;
...
pub fn update_player_by_id(app: &AppHandle, player_id: &str, info: PlayerInfo) -> BGUMCLResult<()> {
  let account_binding = app.state::<Mutex<AccountInfo>>();
  let mut account_state = account_binding.lock()?;
