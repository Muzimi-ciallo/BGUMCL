...
fn resolve_java_home(path: PathBuf) -> BGUMCLResult<String> {
  #[cfg(target_os = "windows")]
...
pub async fn build_mojang_java_download_params(
  ...
  let json =
    json.ok_or_else(|| BGUMCLError("Failed to fetch Mojang Java runtime manifest".into()))?;
  let manifest_url = json[platform][runtime_type][0]["manifest"]["url"]
    .as_str()
    .ok_or_else(|| BGUMCLError("Failed to parse manifest URL".into()))?;
  ...
  let download_params: Vec<_> = manifest["files"]
    .as_object()
    .ok_or_else(|| BGUMCLError("Invalid files data".into()))?
    .iter()
