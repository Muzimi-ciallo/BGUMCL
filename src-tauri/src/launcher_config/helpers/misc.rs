...
impl LauncherConfig {
  pub fn setup_with_app(&mut self, app: &AppHandle) -> BGUMCLResult<()> {
    ...
    self.basic_info = BasicInfo {
      ...
      build_type,
      build_commit_sha: option_env!("BGUMCL_COMMIT_SHA").unwrap_or("").to_string(),
    };
