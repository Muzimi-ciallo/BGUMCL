...
pub fn setup_with_app(app: AppHandle) -> BGUMCLResult<()> {
  ...
  app
    ...
    .map_err(|e| BGUMCLError(format!("Failed to setup log plugin: {}", e)))?;

  Ok(())
}

pub async fn purge_old_launcher_logs(app: AppHandle, days: u64) -> BGUMCLResult<()> {
  let folder = get_launcher_logs_folder(&app);
  if !folder.exists() {
    return Ok(());
  }
