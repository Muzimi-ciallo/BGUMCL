...
#[tauri::command]
pub fn retrieve_memory_info() -> BGUMCLResult<MemoryInfo> {
  Ok(get_memory_info())
}

#[tauri::command]
pub fn retrieve_resolution_upbound(app: AppHandle) -> BGUMCLResult<(u32, u32)> {
  let monitors = app
  ...
  monitors
    ...
    .ok_or_else(|| BGUMCLError("No monitor available".into()))
}

#[tauri::command]
pub fn retrieve_truetype_font_list() -> BGUMCLResult<Vec<String>> {
...
#[tauri::command]
pub fn extract_filename(path_str: String, with_ext: bool) -> BGUMCLResult<String> {
...
#[tauri::command]
pub fn delete_file(path: String) -> BGUMCLResult<()> {
  fs::remove_file(&path).map_err(Into::into)
}

#[tauri::command]
pub fn delete_directory(path: String) -> BGUMCLResult<()> {
  fs::remove_dir_all(&path).map_err(Into::into)
}

#[tauri::command]
pub fn read_file(path: String, mode: Option<String>) -> BGUMCLResult<String> {
  match mode.unwrap_or_else(|| "string".to_string()).as_str() {
    "string" => fs::read_to_string(&path).map_err(Into::into),
    "base64" => fs::read(&path)
      .map(|bytes| general_purpose::STANDARD.encode(bytes))
      .map_err(Into::into),
    value => Err(BGUMCLError(format!("Unsupported mode: {value}"))),
  }
}

#[tauri::command]
pub fn write_file(path: String, content: String, mode: Option<String>) -> BGUMCLResult<()> {
