...
pub async fn get_invalid_library_files(
  ...
  let results: Vec<BGUMCLResult<Option<PTaskParam>>> = join_all(futs).await;
...
#[cfg(target_os = "windows")]
pub fn get_windows_mesa_loader_library(app: &AppHandle) -> BGUMCLResult<Option<LibrariesValue>> {
...
pub fn parse_library_name(name: &str, native: Option<String>) -> BGUMCLResult<LibraryParts> {
...
pub fn convert_library_name_to_path(name: &str, native: Option<String>) -> BGUMCLResult<String> {
  let LibraryParts {
    path,
    pack_name,
    pack_version,
...
pub async fn extract_native_libraries(
  ...
  let tasks: Vec<tokio::task::JoinHandle<BGUMCLResult<()>>> = native_libraries
    .into_iter()
    .map(|library_path| {
  ...
  for result in results {
    if let Err(e) = result {
      ...
      return Err(sjmcl_types::error::BGUMCLError::from(e));
    }
  }
  ...
  let futs = asset_index.objects.into_values().map(|item| {
    ...
    async move {
      ...
      if exists && (!check_hash || validate_sha1(dest.clone(), item.hash.clone()).is_ok()) {
        Ok::<Option<PTaskParam>, sjmcl_types::error::BGUMCLError>(None)
      } else {
        let src = assets_download_api
          .join(&path_in_repo)
          .map_err(sjmcl_types::error::BGUMCLError::from)?;
        Ok(Some(PTaskParam::Download(DownloadParam {
          src,
          dest,
          filename: None,
