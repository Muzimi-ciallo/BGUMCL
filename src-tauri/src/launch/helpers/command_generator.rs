...
impl LaunchArguments {
  pub fn into_hashmap(self) -> BGUMCLResult<HashMap<String, String>> {
    let value =
      serde_json::to_value(self).map_err(|e| BGUMCLError(format!("Serialization error: {}", e)))?;
    let obj = value
      .as_object()
      .ok_or_else(|| BGUMCLError("Failed to convert LaunchParams to HashMap".to_string()))?;
