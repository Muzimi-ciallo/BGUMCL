...
pub enum ModLoaderStatus {
  ...
  DownloadFailed, /* mod loader's library download process failed (including processor installation failed)
                  Only when BGUMCL restart, it will try to re-download library while making no changes to client info JSON (is_retry = true),
...
structstruck::strike! {
  ...
  pub struct Instance {
    ...
    pub modpack_update_channel: Option<String>,
  }
}
...
impl InstanceSummary {
  pub async fn from_instance(
    ...
    InstanceSummary {
      ...
      // ref: https://github.com/UNIkeEN/BGUMCL/pull/799
      ...
      modpack_update_channel: instance.modpack_update_channel.clone(),
    }
  }
}
