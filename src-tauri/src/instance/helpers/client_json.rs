...
impl InstructionRule {
  pub fn is_allowed(&self, target_feature: &FeaturesInfo) -> BGUMCLResult<(bool, bool)> {
...
fn rules_is_allowed(rules: &Vec<InstructionRule>, feature: &FeaturesInfo) -> BGUMCLResult<bool> {
...
pub trait IsAllowed {
  fn is_allowed(&self, feature: &FeaturesInfo) -> BGUMCLResult<bool>;
}
...
impl LaunchArgumentTemplate {
  pub fn to_jvm_arguments(&self, feature: &FeaturesInfo) -> BGUMCLResult<Vec<String>> {
  ...
  pub fn to_game_arguments(&self, feature: &FeaturesInfo) -> BGUMCLResult<Vec<String>> {
    let mut arguments = Vec::new();
...
pub fn load_native_libraries_replace_map(
  app: &AppHandle,
) -> BGUMCLResult<HashMap<String, HashMap<String, Option<LibrariesValue>>>> {
  let path = get_app_resource_filepath(app, "assets/game/natives.json")?;
  let txt =
    fs::read_to_string(&path).map_err(|e| BGUMCLError(format!("read natives.json failed: {e}")))?;
  let map: HashMap<String, HashMap<String, Option<LibrariesValue>>> = serde_json::from_str(&txt)
    .map_err(|e| BGUMCLError(format!("parse natives.json failed: {e}")))?;
  Ok(map)
}
