...
pub async fn load_servers_info_from_nbt(path: &Path) -> BGUMCLResult<Vec<GameServerInfo>> {
  if !path.exists() {
    return Ok(Vec::new());
  }
...
pub async fn save_servers_to_nbt(path: &Path, servers: &[GameServerInfo]) -> BGUMCLResult<()> {
