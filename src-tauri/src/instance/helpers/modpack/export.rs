...
pub fn list_files(instance: &Instance) -> BGUMCLResult<ModpackFileList> {
...
) -> BGUMCLResult<ModpackExportBundle> {
  ...
  tokio::task::spawn_blocking(move || -> BGUMCLResult<()> {
    ...
    for (name, content) in extra_files {
      writer
        ...
        .map_err(|e| BGUMCLError(format!("Failed to create zip entry: {}", e)))?;
      writer
        .write_all(content.as_bytes())
        .map_err(|e| BGUMCLError(format!("Failed to write extra file to zip: {}", e)))?;
    }
    ...
    for (rel, full) in overrides_files {
      ...
      let file = std::fs::File::open(&full)
        .map_err(|e| BGUMCLError(format!("Failed to open file {}: {}", full.display(), e)))?;
