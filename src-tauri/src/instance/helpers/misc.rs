  ...
  let paths = directory_types
    ...
    .map(|directory_type| {
      let path_buf = match directory_type {
        ...
        // native libraries extracted by BGUMCL
...
pub fn unify_instance_name(src_version_path: &PathBuf, tgt_name: &String) -> BGUMCLResult<PathBuf> {
...
pub fn create_instance_shortcut_icon(
  ...
  let mut img = if icon_src == "custom" {
    ...
    let asset = app
      ...
      .ok_or_else(|| {
        sjmcl_types::error::BGUMCLError(format!("Icon asset not found: {}", icon_src))
      })?;
