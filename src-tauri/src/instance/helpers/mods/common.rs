...
pub trait LocalModMetadataParser {
  ...
  ) -> BGUMCLResult<Self::Metadata>;

  async fn get_mod_metadata_from_dir(dir_path: &Path) -> BGUMCLResult<Self::Metadata>;
...
) -> BGUMCLResult<LocalModInfo> {
  let file = Cursor::new(tokio::fs::read(path).await?);
  let file_name = path
    .file_name()
    .map(|name| name.to_string_lossy().to_string())
    .ok_or_else(|| BGUMCLError(format!("invalid mod file path: {}", path.display())))?;
