...
pub async fn load_level_data_from_nbt(path: &PathBuf) -> BGUMCLResult<LevelData> {
  let nbt_bytes = tokio::fs::read(path).await?;
  let (level, _) = deserialize::<Level>(&nbt_bytes, Flavor::GzCompressed)?;
  Ok(level.data)
}

fn level_data_to_world_info(data: &LevelData) -> BGUMCLResult<(i64, String, String)> {
