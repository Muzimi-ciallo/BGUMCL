...
pub async fn install_fabric_loader(
  ...
  let meta = meta.ok_or(BGUMCLError("failed to fetch Fabric meta".to_string()))?;
  let maven_root = maven_root.ok_or(BGUMCLError("failed to get Fabric Maven URL".to_string()))?;

  let loader_path = meta["loader"]["maven"]
    .as_str()
    .ok_or(BGUMCLError("meta missing loader maven".to_string()))?;

  let int_path = meta["intermediary"]["maven"]
    .as_str()
    .ok_or(BGUMCLError("meta missing intermediary maven".to_string()))?;

  let main_class = meta["launcherMeta"]["mainClass"]["client"]
    .as_str()
    .ok_or(BGUMCLError("missing mainClass.client".to_string()))?;
  ...
  let mut push_task = |coord: &str, url_root: &Url| -> BGUMCLResult<()> {
...
pub async fn remove_fabric_api_mods<P: AsRef<Path>>(mods_dir: P) -> BGUMCLResult<()> {
  let mods_dir = mods_dir.as_ref();
  if !mods_dir.exists() {
    return Ok(());
  }
  let re = Regex::new(r"(?i)^(fabric-api|quilted-fabric-api)-.*\.jar$")
    .map_err(|e| BGUMCLError(format!("Invalid regex: {}", e)))?;
  ...
  for p in targets {
    let name = p
      ...
      .unwrap_or_default()
      .to_string();
    fs::remove_file(&p).map_err(|e| BGUMCLError(format!("Failed to remove {}: {}", name, e)))?;
  }
