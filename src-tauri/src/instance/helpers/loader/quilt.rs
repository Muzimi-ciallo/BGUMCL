...
pub async fn install_quilt_loader(
  ...
  let meta = meta.ok_or(BGUMCLError("failed to fetch Quilt loader meta".to_string()))?;
  let quilt_maven = quilt_maven.ok_or(BGUMCLError("failed to get Quilt Maven URL".to_string()))?;
  ...
  let hashed_path = meta["hashed"]["maven"]
    .as_str()
    .ok_or(BGUMCLError("meta missing hashed maven".to_string()))?;
    ...
    if let Some(arr) = launcher_meta.get(side).and_then(|v| v.as_array()) {
      for item in arr {
        if let Some(name) = item["name"].as_str() {
          ...
          let src = src_opt.ok_or(BGUMCLError(format!(
