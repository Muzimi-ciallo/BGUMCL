...
pub fn load_resourcepack_from_zip(path: &PathBuf) -> BGUMCLResult<(String, Option<RgbaImage>)> {
...
pub async fn load_resourcepack_from_dir(path: &Path) -> BGUMCLResult<(String, Option<RgbaImage>)> {
  let mut description = String::new();
  ...
  if let Ok(mut contents) = tokio::fs::read_to_string(path.join("pack.mcmeta")).await {
    ...
    return Err(BGUMCLError("pack.mcmeta not found in ''".to_string()));
  }
