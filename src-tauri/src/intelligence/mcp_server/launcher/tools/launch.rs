...
pub fn tool_routes() -> Vec<ToolRoute<McpContext>> {
  vec![mcp_tool!(
    ...
    "Launch a specific Minecraft instance by its instance_id via BGUMCL deeplink. Before launch, check and update, if user requested, the selected player in launcher configuration.",
    |params|
