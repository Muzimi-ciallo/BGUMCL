...
) -> Result<ModLoaderType, sjmcl_types::error::BGUMCLError> {
  match loader_type {
...
) -> Result<ModLoaderResourceInfo, sjmcl_types::error::BGUMCLError> {
...
) -> Result<Option<OptiFineResourceInfo>, sjmcl_types::error::BGUMCLError> {
    ...
    mcp_tool!(
      ...
      |_app, _params: rmcp::model::JsonObject| async move {
        Ok(serde_json::json!({
          ...
          "automaticDefaultDescription": "When `create_instance.icon_src` is omitted, BGUMCL chooses a default icon from OptiFine, the selected mod loader, or the game version type.",
        }))
      }
    ),
    mcp_tool!(
      "create_instance",
      "Create a Minecraft instance and schedule required client/mod-loader downloads. Resolves game and loader metadata from version IDs. Use `retrieve_instance_icon_options` when choosing a manual `icon_src`. If `icon_src` is omitted, BGUMCL chooses a default icon from OptiFine, the selected mod loader, or the game version type.",
      |app, params|
