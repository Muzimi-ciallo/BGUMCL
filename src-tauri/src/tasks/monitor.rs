...
impl TaskMonitor {
  ...
  pub async fn enqueue_task<T>(
    ...
    T: Future<Output = BGUMCLResult<()>> + Send + 'static,
  {
  ...
  pub async fn enqueue_task_group(&self, task_group: String, futures: Vec<BGUMCLFutureDesc>) {
  ...
  /// Wait until a task group reaches a terminal state. Used by commands that
  /// schedule downloads and then need to perform cleanup/finalization steps.
  pub async fn wait_for_group_completion(&self, task_group: &str) -> BGUMCLResult<()> {
    loop {
      let status = {
        let group_map = self.group_map.read().unwrap();
        match group_map.get(task_group) {
          Some(group) => group.status.clone(),
          None => {
            return Err(BGUMCLError(format!(
              "Task group disappeared: {}",
              task_group
            )));
          }
        }
      ...
      match status {
        GEventStatus::Completed => return Ok(()),
        GEventStatus::Failed => {
