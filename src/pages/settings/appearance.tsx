...
const AppearanceSettingsPage = () => {
      ...
      if (response.status === "success") {
        ...
        if (fileName === selectedBgKey) {
          ...
          if (customBgList.length === 1) {
            newSelectedBgKey = `${builtInBgPrefix}BGUMCL`;
  ...
  const PresetBackgroundList = () => {
    const presetBgList = [
      { key: "BGUMCL", name: "BGUMCL" },
      { key: "taoyuan", name: "暑期一周目桃园镇" },
      { key: "wandagate", name: "暑期一周目湾大校门" },
    ];
      ...
      <Wrap spacing={3.5} justify="right">
        {presetBgList.map((bg) => (
          <WrapItem key={bg.key}>
            <BackgroundCard
              bgAlt={bg.name}
              bgSrc={`/images/backgrounds/${bg.key}.jpg`}
  ...
  const appearanceSettingGroups: OptionItemGroupProps[] = [
...

    {
      ...
      items: [
        ...
        // font size settings cannot work in Windows now: https://github.com/UNIkeEN/BGUMCL/issues/376
