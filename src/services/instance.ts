...
export class InstanceService {
  ...
  /**
   * CHECK for a GitHub-based modpack incremental update.
   * @param {string} instanceId - The ID of the instance.
   * @returns {Promise<InvokeResponse<GithubModpackUpdateInfo>>}
   */
  @responseHandler("instance")
  static async checkGithubModpackUpdate(
    instanceId: string
  ): Promise<InvokeResponse<GithubModpackUpdateInfo>> {
    return await invoke("check_github_modpack_update", {
      instanceId,
    });
  }

  /**
   * APPLY a GitHub-based modpack incremental update (downloads changed files only).
   * @param {string} instanceId - The ID of the instance.
   * @returns {Promise<InvokeResponse<GithubModpackUpdateInfo>>}
   */
  @responseHandler("instance")
  static async applyGithubModpackUpdate(
    instanceId: string
  ): Promise<InvokeResponse<GithubModpackUpdateInfo>> {
    return await invoke("apply_github_modpack_update", {
      instanceId,
    });
  }

  /**
   * SET the GitHub modpack update manifest URL for an instance.
   * @param {string} instanceId - The ID of the instance.
   * @param {string} manifestUrl - Raw JSON manifest URL (e.g. raw.githubusercontent.com/.../sjmcl-update.json).
   * @returns {Promise<InvokeResponse<void>>}
   */
  @responseHandler("instance")
  static async setGithubModpackUpdateChannel(
    instanceId: string,
    manifestUrl: string
  ): Promise<InvokeResponse<void>> {
    return await invoke("set_github_modpack_update_channel", {
      instanceId,
      manifestUrl,
    });
  }
