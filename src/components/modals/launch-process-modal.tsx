...
const LaunchProcessModal: React.FC<LaunchProcessModalProps> = ({
  ...
  const launchProcessSteps: Array<{
    ...
    () => [
      ...
      {
        label: "checkModpackUpdate",
        function: () => {
          if (!effectiveInstance?.modpackUpdateChannel) {
        ...
        isOK: (data: GithubModpackUpdateInfo | null) =>
          !data ||
          (data.filesToDownload.length === 0 &&
            data.filesToRemove.length === 0),
        onResCallback: (data: any) => {
          const info = data as GithubModpackUpdateInfo | null;
          if (!info) {
            setActiveStep((step) => step + 1);
            return;
          }
          openGenericConfirmDialog({
            title: t("LaunchProcessModal.modpackUpdate.title"),
            body: t("LaunchProcessModal.modpackUpdate.body", {
              version: info.latestVersion,
              count: info.filesToDownload.length,
            }),
            btnOK: t("LaunchProcessModal.modpackUpdate.updateAndLaunch"),
            btnCancel: t("LaunchProcessModal.modpackUpdate.launchOnly"),
            onOKCallback: () => {
              InstanceService.applyGithubModpackUpdate(
                effectiveInstance?.id || ""
              ).then((response) => {
                if (response.status === "success") {
                  ...
                  toast({ title: response.details, status: "error" });
                }
                setActiveStep((step) => step + 1);
              });
            },
        ...
        onErrCallback: (error: ResponseError) => {
          toast({ title: error.details, status: "warning" });
          setActiveStep((step) => step + 1);
        },
      },
      {
